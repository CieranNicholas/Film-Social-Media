...
export const getReviewsByMediaId = async (mediaId: number): Promise<dbPromise> => {
	try {
		const reviews = await prisma.review.findMany({
			where: {
				mediaId: mediaId,
...
export const getAvarageReviewRating = async (mediaId: number): Promise<dbPromise> => {
	...
	try {
		...
		return {
			...
			success: true,
			data: reviews.reduce((acc, review) => acc + review.rating, 0) / reviews.length,
		};
	} catch (e) {
		if (e instanceof Prisma.PrismaClientKnownRequestError) {
			console.error(e.message);
...
export const getPopularReviewsByMediaId = async (mediaId: number): Promise<dbPromise> => {
	if (!mediaId) return { message: 'No mediaId provided', success: false };
	try {
		const reviews = await prisma.review.findMany({
			where: {
...
export const getUserDataFromUsername = async (username: string): Promise<dbPromise> => {
	try {
		...
		const user = users.find((user) => user.username?.toLowerCase() === username.toLocaleLowerCase());
		if (user) {
			return {
				message: 'User Data Fetched Successfully',
				success: true,
...
export const followUser = async (userId: string, followingId: string): Promise<dbPromise> => {
	try {
...
export const unfollowUser = async (userId: string, followingId: string): Promise<dbPromise> => {
	try {
...
export async function AddFavouriteFilm(uid: string, title: string, movieId: number, posterPath: string): Promise<dbPromise> {
	try {
...
export async function deleteFavouriteFilm(uid: string, dbId: string): Promise<dbPromise> {
	try {
...
export async function AddFavouriteTv(uid: string, title: string, tvId: number, posterPath: string): Promise<dbPromise> {
	try {
...
export async function deleteFavouriteTv(uid: string, dbId: string): Promise<dbPromise> {
	try {
...
export const createUser = async (name: string, username: string, email: string, password: string): Promise<dbPromise> => {
...
export const updateUsername = async (userId: string, newUsername: string): Promise<dbPromise> => {
	try {
