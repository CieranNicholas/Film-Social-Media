...
const ModalNewReview = () => {
	...
	const [selectedItem, setSelectedItem] = useState<Movie | undefined>(undefined);
	...
	const submitReview = async () => {
		...
		const title = selectedItem.title || selectedItem.original_title || selectedItem.original_name;
		const releaseDate = selectedItem.release_date || selectedItem.first_air_date;
		const res = await createReview(session.user.id, id, posterPath, title, reviewContent, rating, selectedType, releaseDate);
		if (!res.success) {
			onClose();
			toast({
						...
						>
							...
							<>
								{movieResult.map((movie) => (
									...
									>
										<div className='flex flex-col items-start justify-center w-[90%]' onClick={() => selectItem(movie, 'MOVIE')}>
							...
							<>
								{tvResult.map((tv) => (
									<div
										...
										className='flex justify-between items-center border-b border-neutral-700 cursor-pointer group transition-all hover:border-primary'
									>
										<div className='flex flex-col items-start justify-center w-[90%]' onClick={() => selectItem(tv, 'TV')}>
											<p className='font-semibold group-hover:text-primary transition-all'>
				...
				<AnimatePresence>
					{selectedItem && (
						<motion.div
							...
							exit={{ opacity: 0 }}
						>
							<img src={`http://image.tmdb.org/t/p/w92${selectedItem.poster_path}`} className='rounded-lg' />
							<div className='flex flex-col items-start justify-center'>
								<p className='font-semibold'>{selectedItem.title || selectedItem.original_title || selectedItem.original_name}</p>
								<p className='text-sm text-neutral-500'>{selectedItem.release_date || selectedItem.first_air_date}</p>
								<p className='text-sm'>{truncateText(selectedItem.overview, 50)}</p>
							</div>
						</motion.div>
					)}
				</AnimatePresence>
...
function StarRating({ value, onChange }: { value: number; onChange: (value: number) => void }) {
	...
	return <div className='flex w-full items-center justify-start gap-4'>{stars}</div>;
}
