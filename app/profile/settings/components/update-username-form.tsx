...
const UpdateUsernameForm: React.FC<UpdateUsernameFormProps> = ({ user, setUser }) => {
	...
	const handleUpdateUsername = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (res?.success) {
			...
			setNewUsername('');
			setUser((value: UserDataType | undefined) => (value ? { ...value, username: newUsername } : undefined));
		}
		setIsUsernameUpdating(false);
	};

	return (
		<form className='text-slate-100 p-2 rounded shadow flex flex-col gap-2 justify-center items-start' onSubmit={handleUpdateUsername}>
