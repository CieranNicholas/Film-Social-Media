...
const Hero: React.FC<HeroProps> = ({ movie, videos, avarageRating }) => {
	...
	const { data, loading, error } = useColor(`https://media.themoviedb.org/t/p/w92/${movie.poster_path}`, 'rgbArray', {
	...
	useEffect(() => {
		const youtubeFiltered = videos.results.filter((video: any) => video.site === 'YouTube');
		const trailers = youtubeFiltered.filter((video: any) => video.type === 'Trailer');
	...
	return (
		<section
			className={textColor === 'white' ? 'w-full bg-cover bg-no-repeat text-white' : 'w-full bg-cover bg-no-repeat text-neutral-900'}
		...
		>
			{!loading ? (
				...
				>
					<motion.div className='flex justify-center items-center p-10 pt-32'>
						...
						<div className='flex flex-col gap-4 items-start justify-center h-[450px]'>
							...
							<div className='flex flex-col gap-2 lg:gap-6 md:flex-row'>
								...
								<span className='flex gap-1 items-center justify-start lg:list-item'>{formatMinutes(movie.runtime)}</span>
							</div>
							...
							<p>{movie.overview}</p>
							<Button className='mt-auto' variant='secondary' onClick={() => onOpen(trailer?.key)}>
								Trailer
							</Button>
						</div>
					</motion.div>
				...
				<div className='w-full h-[618px]'></div>
			)}
		</section>
	);
};
