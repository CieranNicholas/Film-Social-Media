...
const Following = () => {
  ...
  const [following, setFollowing] = useState<UserDataType[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const following of user.following) {
        const res = await getUserDataFromId(following.followingId);
        if (res.success) {
          setFollowing((prev) => {
  ...
  return (
    <main className='flex justify-center items-start px-4 pt-16 pb-8 mx-auto md:px-0 md:pt-32 md:pb-16 bg-background h-full w-full text-white min-h-[100vh]'>
      <div className='flex flex-col gap-4 w-full md:w-2/3 xl:w-1/2'>
        ...
        >
          {following.length > 0 ? (
            <FollowList list={following} data={data} user={user} />
          ) : (
            <p className='m-auto text-white/80'>
              Looks like this user is not following anyone yet 😔
            </p>
          )}
        </motion.section>
      </div>
